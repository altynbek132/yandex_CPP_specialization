...
class AirportCounter {
 private:
  constexpr uint32_t static AirportN(TAirport);
 
 public:
  static const size_t SIZE = AirportN(TAirport::Last_);
 ...
 private:
  ...
  TAirport static NAirport(uint32_t);
};
...
typename AirportCounter<TAirport>::Items AirportCounter<TAirport>::GetItems() const {
    ...
    for (std::size_t i = 0; i < SIZE; ++i) {
        items[i] = {NAirport(i), airports[i]};
    }
    return items;
}

template<typename TAirport>
constexpr uint32_t AirportCounter<TAirport>::AirportN(TAirport airport) {
    return static_cast<uint16_t>(airport);
}

template<typename TAirport>
TAirport AirportCounter<TAirport>::NAirport(uint32_t N) {
    return static_cast<TAirport>(N);
}


template<typename TAirport>
void AirportCounter<TAirport>::Clear() {
    airports.fill(0);
}
