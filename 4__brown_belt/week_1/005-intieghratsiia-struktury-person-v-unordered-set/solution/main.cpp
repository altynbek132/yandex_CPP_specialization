...
struct AddressHasher {
    size_t operator()(const Address& address) const {
        ...
        return (coef * coef * string_hasher(address.city) + coef * string_hasher(address.street) +
...
struct PersonHasher {
    size_t operator()(const Person& person) const {
        ...
        return (coef * coef * coef * string_hasher(person.name) + coef * coef * int_hasher(person.height) +
                coef * double_hasher(person.weight) + address_hasher(person.address));
    }
};
