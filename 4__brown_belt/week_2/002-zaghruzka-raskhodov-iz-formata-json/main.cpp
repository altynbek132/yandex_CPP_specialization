...
vector<Spending> LoadFromJson(istream& input) {
    auto doc = Load(input);
    vector<Spending> res;
    for (auto& entry : doc.GetRoot().AsArray()) {
        auto& m = entry.AsMap();
        res.push_back({m.at("category").AsString(), m.at("amount").AsInt()});
    }
    return res;
}
...
void TestLoadFromJson() {
    ...
    const vector<Spending> expected = {{"food", 2500},    {"transport", 1150}, {"restaurants", 5780},
