...
Json::Document XmlToJson(const Xml::Document& doc) {
    ...
    for (const auto& n : doc.GetRoot().Children()) {
        result.emplace_back(map<string, Node>{{"category", Node{n.AttributeValue<string>("category")}},
...
void TestJsonToXml() {
    ...
    const Document json_doc{Node(vector<Node>{
        Node(map<string, Node>{{"category", Node("food")}, {"amount", Node(2500)}}),
        Node(map<string, Node>{{"category", Node("transport")}, {"amount", Node(1150)}}),
        Node(map<string, Node>{{"category", Node("restaurants")}, {"amount", Node(5780)}}),
        Node(map<string, Node>{{"category", Node("clothes")}, {"amount", Node(7500)}}),
        Node(map<string, Node>{{"category", Node("travel")}, {"amount", Node(23740)}}),
        Node(map<string, Node>{{"category", Node("sport")}, {"amount", Node(12000)}}),
    })};
    ...
    const vector<string> expected_category = {"food",    "transport", "restaurants",
                                              "clothes", "travel",    "sport"};
