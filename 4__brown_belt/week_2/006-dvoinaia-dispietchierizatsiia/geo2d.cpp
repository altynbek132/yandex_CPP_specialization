...
bool Collide(Point p, Segment s) {
    ...
    return ScalarProduct(v1, Vector{s.p1, s.p2}) >= 0 && ScalarProduct(v2, Vector{s.p2, s.p1}) >= 0 &&
...
bool Collide(Rectangle r, Segment s) {
    return Collide(s.p1, r) || Collide(s.p2, r) || Collide(s, Segment{r.BottomLeft(), r.BottomRight()}) ||
           Collide(s, Segment{r.BottomRight(), r.TopRight()}) ||
           Collide(s, Segment{r.TopRight(), r.TopLeft()}) || Collide(s, Segment{r.TopLeft(), r.BottomLeft()});
}
...
bool Collide(Rectangle r, Circle c) {
    return Collide(c.center, r) || Collide(c, Segment{r.BottomLeft(), r.BottomRight()}) ||
           Collide(c, Segment{r.BottomRight(), r.TopRight()}) ||
           Collide(c, Segment{r.TopRight(), r.TopLeft()}) || Collide(c, Segment{r.TopLeft(), r.BottomLeft()});
}
