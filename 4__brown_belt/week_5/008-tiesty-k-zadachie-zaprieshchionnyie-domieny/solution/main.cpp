...
class DomainChecker {
   ...
   private:
    ...
    static bool IsDomainLess(const Domain* lhs, const Domain* rhs) {
        ...
        return lexicographical_compare(begin(lhs_reversed_parts), end(lhs_reversed_parts),
...
void TestDomainReversedParts() {
    ...
    const vector<string_view> reversed_parts(begin(reversed_parts_range), end(reversed_parts_range));
    ASSERT_EQUAL(reversed_parts, vector<string_view>({"ru", "ya"}));
}
...
void TestDomainCheckerNoAbsorbation() {
    const vector<Domain> domains = {Domain("ya.ru"), Domain("m.ya.ru")};
    ASSERT_EQUAL(DomainChecker(begin(domains), end(domains)).IsSubdomain(Domain("pets.ya.ru")), true);
}

void TestCheckDomainsInvertBool() {
    ASSERT_EQUAL(CheckDomains(vector({Domain("ya.ru")}), vector({Domain("m.ya.ru")})), vector({false}));
}
...
void TestReadDomainsExcessEmpty() {
    stringstream stream;
    stream << 1 << '\n' << "ya.ru";
    ASSERT_EQUAL(ReadDomains(stream), vector({Domain("ya.ru")}));
}
