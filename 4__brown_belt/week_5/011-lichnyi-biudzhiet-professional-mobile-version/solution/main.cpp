...
struct MoneyState {
    ...
    MoneyState operator+(const MoneyState& other) const { return MoneyState(*this) += other; }

    MoneyState operator*(double factor) const { return {earned * factor, spent * factor}; }
};
...
struct BulkTaxApplier {
    ...
    static BulkTaxApplier FromPercentage(uint8_t percentage) { return {1.0 - percentage / 100.0}; }
};
    ...
    class AddBulkOperationVisitor {
       public:
        ...
        void ProcessPartial(const NodeHolder& node, IndexSegment) const {
            node->data =
                (node->left ? node->left->data : Data()) + (node->right ? node->right->data : Data());
        }
    ...
    static void PropagateBulkOperation(const NodeHolder& node) {
        for (auto* child_ptr : {node->left.get(), node->right.get()}) {
            if (child_ptr) {
                ...
                child_ptr->data =
                    node->postponed_bulk_operation.Collapse(child_ptr->data, child_ptr->segment);
            }
        }
...
struct Request {
    enum class Type { COMPUTE_INCOME, EARN, SPEND, PAY_TAX };
